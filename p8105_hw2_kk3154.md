...
3.2 - Producing a reader-friendly table of the rank in popularity of the
name “Olivia” as a female baby name over time, with rows of ethnicity
and columns of year.
...
baby_df %>% 
  ...
  group_by(ethnicity) %>% 
  fill(`2016`:`2011`, .direction = 'updown') %>% 
  distinct() %>% 
  knitr::kable()
```
...
3.3 - Producing a reader-friendly table of the most popular male baby
name over time.
...
3.4 Producing a scatter plot for male, white, non-Hispanic children born
in 2016 of count of children with that name (Y axis) against rank in
popularity (x axis).
...
![](p8105_hw2_kk3154_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->
